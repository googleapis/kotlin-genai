...
internal constructor(
   ...
   *
   * The turns so far are sent along with [contents], so the model has the conversation for context.
   * The turn joins the history once the flow completes, keeping each chunk as its own entry, so a
   * ten chunk response adds ten model entries rather than one; [getHistory] counts entries, not
   * turns. A stream that is cut off before the model finishes is kept but is not sent again in
   * later requests.
   *
