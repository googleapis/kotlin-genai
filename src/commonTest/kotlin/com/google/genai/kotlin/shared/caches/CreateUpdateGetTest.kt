...
class CreateUpdateGetTest : SharedTestBase() {
  ...
  // The Gemini API does not accept gs:// URIs, so the mldev half of the canonical
  // cache lifecycle uses inline bytes large enough to meet the caching minimum.
  ...
  /** Canonical caches/create_update_get: test_create_update_get. */
  @Test
  fun testCreateUpdateGet() = runTest {
    listOf(false, true).forEach { enterprise ->
      if (skipBackend(enterprise)) return@forEach
      val suffix = if (enterprise) "vertex" else "mldev"
      val testName = "shared.caches.CreateUpdateGetTest.testCreateUpdateGet.$suffix"
        ...
        try {
          val updated =
            client.caches.update(
              name = cache.name!!,
              config = UpdateCachedContentConfig(ttl = 7200.seconds),
            )
          assertEquals(cache.name, updated.name)

          val got = client.caches.get(name = updated.name!!)
