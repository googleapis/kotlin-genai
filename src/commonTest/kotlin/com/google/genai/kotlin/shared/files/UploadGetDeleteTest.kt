...
class UploadGetDeleteTest : SharedTestBase() {
  /**
   * Canonical files/upload_get_delete: test_upload_get_delete_image, Gemini API only.
   *
   * The canonical fixture is a PNG on disk; commonTest has no test-asset directory, so this
   * uploads equivalent inline bytes (the pattern the existing FilesE2ETest already records).
   */
  @Test
      ...
      if (enterprise) {
        val e =
          kotlin
            .runCatching {
              client.files.upload(
                byteArray = "unused".encodeToByteArray(),
                config = UploadFileConfig(mimeType = "text/plain"),
              )
            }
            .exceptionOrNull()
        assertNotNull(e, "Expected the Agent Platform backend to reject files.upload")
        assertTrue(
