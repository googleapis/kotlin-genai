...
/** Canonical GEMINI_MODEL from the cross-SDK shared corpus. */
const val SHARED_MODEL = "gemini-3.1-pro-preview"

/**
 * Base class for the curated "shared" integration tests: the cross-SDK critical pathways mirrored
 * from google/cloud/aiplatform/sdk/genai/replays/tests/shared. These are the tests the nightly
 * API-mode jobs run against the live backends.
 *
...
open class SharedTestBase : BaseTestServer() {
  ...
  /**
   * True when the currently running job has selected the other backend.
   *
   * Mirrors GOOGLE_GENAI_RUN_{VERTEX,GEMINI}_ONLY_IN_API_MODE as used by the Python, Go, Java, JS
   * and .NET shared suites: each nightly job sets exactly one of them. When neither is set (which
   * is what replay/presubmit sees, since build.gradle.kts clears them) both backends run.
   *
   * `kotlin.test` has no runtime "skipped" status, so callers return early instead:
  ...
  /**
   * Runs a live call, swallowing a 429 rather than failing the build.
   *
   * A quota response still proves the SDK built the request, authenticated, routed to the live
   * endpoint and deserialized the error, so it is not a regression. See
