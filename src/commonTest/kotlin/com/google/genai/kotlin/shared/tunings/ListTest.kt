...
class ListTest : SharedTestBase() {
  /**
   * Canonical tunings/list: test_default (pageSize 1).
  ...
  fun testDefault() = runTest {
    listOf(false, true).forEach { enterprise ->
      ...
      if (!enterprise) {
        val e =
          kotlin
            .runCatching { client.tunings.list(ListTuningJobsConfig(pageSize = 1)).take(1).toList() }
            .exceptionOrNull()
        assertNotNull(e, "Expected the Gemini API backend to reject tunings.list")
        assertTrue(
