...
class ListTest : SharedTestBase() {
  /** Canonical caches/list: test_list_cached_contents (pageSize 2). */
  @Test
  fun testListCachedContents() = runTest {
    listOf(false, true).forEach { enterprise ->
      ...
      val testName = "shared.caches.ListTest.testListCachedContents.$suffix"
      val client = createClient(enterprise, testName)

      runLive {
        val pager = client.caches.list(ListCachedContentsConfig(pageSize = 2))
        assertNotNull(pager)
        val items = pager.take(2).toList()
        assertTrue(items.size <= 2, "Expected at most 2 cached contents, got ${items.size}")
      }
    }
  }
}
